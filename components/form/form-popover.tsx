...
export const FormPopover = ({
  ...
  const handleUserExpenseChange = (userId: string, amount: string) => {
    setUserExpenses((prevUserExpenses) => {
      const userIndex = prevUserExpenses.findIndex(
        (user) => user.id === userId
      );
      ...
      if (userIndex !== -1) {
        ...
        const newTotalAmount = newUserExpenses.reduce(
          (total, userExpense) => total + parseFloat(userExpense.amount || "0"),
          0
        );
  ...
  return (
    <Popover>
      ...
      <PopoverContent
        align={align}
        className="w-80 pt-3 mt-1 sm:mt-0 mr-4 shadow-xl"
        side={side}
      ...
      >
        ...
        <PopoverClose asChild ref={closeRef} onClick={resetForm}>
          <Button
            className="h-auto w-auto p-2 mt-0.5 sm:mt-0 mr-4 absolute top-2 right-2 text-neutral-600"
            variant="ghost"
          >
              ...
              <label className="flex w-full gap-2">
                ...
                <input
                  ...
                  value={
                    (
                      userExpenses.find(
                        (userExpense) => userExpense.id === member.userId
                      ) || {}
                    ).amount || ""
                  }
                  onChange={(e) =>
                  ...
                  placeholder="₹"
                  className="border-b w-16 ml-auto text-center focus:outline-none focus:border-black"
                />
              </label>
            </div>
          ))}
